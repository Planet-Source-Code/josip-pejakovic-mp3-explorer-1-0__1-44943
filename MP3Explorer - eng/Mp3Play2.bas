...
Function MP3Play(Mp3File As String)
...
Public Function TrackLength()
Static duzina As String * 30
Dim sekunde As Integer
Dim minute As Integer

mciSendString "set MP3Play Time Format milliseconds", "", 0, 0
mciSendString "status MP3Play length", duzina, Len(duzina), 0

sekunde = Int(Val(Mid$(duzina, 1, Len(duzina))) / 1000)

If sekunde < 60 Then
TrackLenghth = "0:" & Format(sekunde, "00")
Else
minute = Int(sekunde / 60)
sekunde = sekunde - (minute * 60)
TrackLength = Format(minute, "00") & ":" & Format(sekunde, "00")
End If
